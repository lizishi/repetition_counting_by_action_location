...
type2label = ['pull_up', 'others', 'situp', 'battle_rope',
              'pommelhorse', 'jump_jack', 'bench_pressing', 'front_raise', 'push_up', 'squat']
...
def segment_iou_numpy(segment1, segment2):
    ...
    if segment1.ndim == 2 and segment2.ndim == 2:
        inter = np.clip(
            np.min([segment1[:, None, 1] * np.ones_like(segment2[None, :, 1]),
                    np.ones_like(segment1[:, None, 1]) * segment2[None, :, 1]], axis=0) -
            np.max([segment1[:, None, 0] * np.ones_like(segment2[None, :, 0]),
                    np.ones_like(segment1[:, None, 0]) * segment2[None, :, 0]], axis=0),
            a_min=0, a_max=None
        )
        union = (segment1[:, None, 1] - segment1[:, None, 0]) + (segment2[None, :, 1] - segment2[None, :, 0]) - inter
        iou = inter / (union + 1e-6)
    elif segment1.ndim == 3 and segment2.ndim == 3:
        inter = np.max(np.zeros(1), np.min(segment1[:, :, None, 1], segment2[:, None, :, 1]) -
                       np.max(segment1[:, :, None, 0], segment2[:, None, :, 0]))
        union = (segment1[:, :, None, 1] - segment1[:, :, None, 0]) + (
                    segment2[:, None, :, 1] - segment2[:, None, :, 0]) - inter
        iou = inter / (union + 1e-6)
    else:
...
class RepCountDataset(data.Dataset):
    ...
    def __init__(self, prop_file, ft_path, pipeline, exclude_empty=True, epoch_multiplier=1,
                 clip_len=128, class_num=2,
    ...
    def get_all_gt(self):
        ...
        for video in self.video_list:
            gts = [[x.label, (x.start_frame + x.end_frame) / (2 * video.num_frames), (x.end_frame - x.start_frame) / video.num_frames]
                   for x in video.gt]
    ...
    def evaluate(self, result, logger=None, metrics=None):
        ...
        for (pred, gt) in zip(result_pred, result_gt):
            pred_num = len([i for j in range(self.class_num - 1) for i in pred[j] if i[-1] > threshold])
        ...
        for pred in result_pred:
            pred_all = np.array([i for j in range(self.class_num - 1) for i in pred[j] if i[-1] > threshold])
            if len(pred_all) == 0:
                continue
        ...
        return {'mAP': mean_map, "MAE": mae_mean, "OBO": obo_mean, "iou_decay": iou_decay}
